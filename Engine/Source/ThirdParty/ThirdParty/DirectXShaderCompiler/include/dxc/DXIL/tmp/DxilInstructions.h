...
struct DxilInst_ExperimentalNop {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::ExperimentalNop);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_GetGroupWaveIndex {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::GetGroupWaveIndex);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_GetGroupWaveCount {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::GetGroupWaveCount);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_RayQuery_CandidateClusterID {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_RayQuery_CandidateClusterID(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::RayQuery_CandidateClusterID);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_RayQuery_CommittedClusterID {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_RayQuery_CommittedClusterID(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::RayQuery_CommittedClusterID);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_HitObject_ClusterID {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::HitObject_ClusterID);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_TriangleObjectPosition {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::TriangleObjectPosition);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction returns candidate triangle vertices in object space as <9 x float>
struct DxilInst_RayQuery_CandidateTriangleObjectPosition {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_RayQuery_CandidateTriangleObjectPosition(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::RayQuery_CandidateTriangleObjectPosition);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction returns committed triangle vertices in object space as <9 x float>
struct DxilInst_RayQuery_CommittedTriangleObjectPosition {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_RayQuery_CommittedTriangleObjectPosition(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::RayQuery_CommittedTriangleObjectPosition);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_HitObject_TriangleObjectPosition {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_HitObject_TriangleObjectPosition(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::HitObject_TriangleObjectPosition);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Returns the resulting matrix from multiplying A and B and accumulating into C
struct DxilInst_LinAlgMatrixMultiplyAccumulate {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixMultiplyAccumulate(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixMultiplyAccumulate);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgFillMatrix {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgFillMatrix);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Converts and copies the element and use type of the source matrix to the destination matrix with optional transpose
struct DxilInst_LinAlgCopyConvertMatrix {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgCopyConvertMatrix(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgCopyConvertMatrix);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatrixLoadFromDescriptor {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixLoadFromDescriptor(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixLoadFromDescriptor);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatrixLoadFromMemory {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixLoadFromMemory(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixLoadFromMemory);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction returns the number of elements stored in thread-local storage on the active thread for the provided matrix
struct DxilInst_LinAlgMatrixLength {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixLength(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixLength);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction returns a two element vector containing the column and row of the matrix that the thread-local index corresponds to
struct DxilInst_LinAlgMatrixGetCoordinate {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixGetCoordinate(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixGetCoordinate);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction returns the element of the matrix corresponding to the provided thread-local index
struct DxilInst_LinAlgMatrixGetElement {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixGetElement(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixGetElement);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction sets the element of the matrix corresponding to the provided thread-local index
struct DxilInst_LinAlgMatrixSetElement {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixSetElement(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixSetElement);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatrixStoreToDescriptor {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixStoreToDescriptor(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixStoreToDescriptor);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatrixStoreToMemory {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixStoreToMemory(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixStoreToMemory);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction returns comptime 0 when accumulator matrix are A layout, 1 when B layout
struct DxilInst_LinAlgMatrixQueryAccumulatorLayout {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixQueryAccumulatorLayout(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixQueryAccumulatorLayout);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatrixMultiply {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixMultiply);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction accumulate A or B matrix into Accumulator matrix following LHS += RHS
struct DxilInst_LinAlgMatrixAccumulate {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixAccumulate(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixAccumulate);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatVecMul {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatVecMul);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Multiplies a MxK dimension matrix and a K sized input vector then adds a M sized bias vector
struct DxilInst_LinAlgMatVecMulAdd {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatVecMulAdd(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatVecMulAdd);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatrixAccumulateToDescriptor {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixAccumulateToDescriptor(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixAccumulateToDescriptor);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgMatrixAccumulateToMemory {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixAccumulateToMemory(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixAccumulateToMemory);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Outer products an M sized vector and a N sized vector producing an MxN matrix
struct DxilInst_LinAlgMatrixOuterProduct {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgMatrixOuterProduct(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgMatrixOuterProduct);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Convert vector components from one interpretation to another
struct DxilInst_LinAlgConvert {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgConvert(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgConvert);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_LinAlgVectorAccumulateToDescriptor {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_LinAlgVectorAccumulateToDescriptor(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::LinAlgVectorAccumulateToDescriptor);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_IsDebuggerPresent {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr, hlsl::OP::OpCode::IsDebuggerPresent);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
