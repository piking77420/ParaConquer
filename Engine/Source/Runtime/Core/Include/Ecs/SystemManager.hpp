...
BEGIN_PCCORE
    ...
    public:
        ...
        PC_FORCE_INLINE void EntityDestroyed(EntityId entityId, const Signature& entityIdSignature)
        {
            for (auto& it : m_Systems)
                it->OnEntityDestroy(entityId, entityIdSignature);
        }

        PC_FORCE_INLINE void EntitySignatureChanged(EntityId entityId, const Signature& _oldSignature,
                                                    const Signature& _newSignature)
        {
            for (auto& it : m_Systems)
