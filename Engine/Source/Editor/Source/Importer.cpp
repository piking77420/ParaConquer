...
bool PC_EDITOR_CORE::Importer::ImportTexture(const std::filesystem::path& _path, PC_CORE::Serializer* _serializer, PC_CORE::TypeId* _outId, PC_CORE::ObjectPtr<PC_CORE::Resource>* _outResource) const
{
	...
	PC_CORE::ObjectPtr<PC_CORE::Resource>* _outResource) const
{
	...
	*_outResource = PC_CORE::ResourceManager::Create<PC_CORE::StaticMesh>(staticMeshCreateInfo);
	...
	indicies.CompressData(staticMeshCreateInfo.staticMeshRenderData.indices.data(), sizeof(staticMeshCreateInfo.staticMeshRenderData.indices[0]) * staticMeshCreateInfo.staticMeshRenderData.indices.size());
