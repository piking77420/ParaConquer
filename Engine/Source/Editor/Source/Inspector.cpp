    ...
    std::visit([&](auto&& arg) 
        {
            ...
            {
                const PC_CORE::ReflectedWeakObjPtr& wkPtr = std::get<PC_CORE::ReflectedWeakObjPtr>(type.metaData.data);
...
void Inspector::HandlePtr(uint8_t* ptr, const PC_CORE::ReflectedType& type, const PC_CORE::Members& _typeAsMember)
{
    ...
    PC_CORE::WeakObjectPtr<PC_CORE::Resource>* doublePtr = reinterpret_cast<PC_CORE::WeakObjectPtr<PC_CORE::Resource>*>(ptr);
    ...
    if (showSelectResourceMenue && currentSelectedMember == _typeAsMember.membersName)
    {
        ...
        PC_CORE::ResourceManager::ForEach(std::get<PC_CORE::ReflectedWeakObjPtr>(type.metaData.data).typeId, l);
