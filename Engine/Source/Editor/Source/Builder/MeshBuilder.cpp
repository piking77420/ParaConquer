		...
		bool Optimise)
	{
        ...
        for (auto& f : futures)
        {
            ...
            MeshDescriptorsData.emplace_back(MeshDescriptor
                {
                    .VertexOffset = static_cast<uint32_t>(Data.OutVertices.size()),
                    .VertexCount = static_cast<uint32_t>(Optimised.Verticies.size()),
                    .IndicesOffset = static_cast<uint32_t>(Data.OutIndices.size()),
                    .IndiceCount = static_cast<uint32_t>(Optimised.Indicies.size()),
                });
