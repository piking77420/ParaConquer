...
void FileSystemWatcher::WorkerMainLoop()
{
    ...
    assert(m_Worker.fileHandle != nullptr);

    std::array<FILE_NOTIFY_INFORMATION, 1024>& fileNotificationBuffer = *reinterpret_cast<std::array<
        FILE_NOTIFY_INFORMATION, 1024>*>(m_Worker.systemBuffer.get());
    ...
    while (m_Worker.watch)
    {
        ...
        DWORD byteReturn;
        BOOL b = ReadDirectoryChangesW(m_Worker.fileHandle, fileNotificationBuffer.data(),
                                       fileNotificationBuffer.size() * sizeof(FILE_NOTIFY_INFORMATION),
                                       TRUE, flag, &byteReturn, reinterpret_cast<OVERLAPPED*>(&m_Worker.asyncObj[0]),
                                       nullptr);

        auto pNotify = fileNotificationBuffer.data();
        ...
        for (;;)
        {
            ...
            bool isFormatHandle = IsValidFormat(m_ExpectFormat.data(), m_ExpectFormat.size(),
                                                m_Worker.fileNameBuffer.data(), &format);
