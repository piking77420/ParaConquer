...
void VulkanShaderManager::CreateShaderResourceFromSpvReflectModule(vk::Device _device, ShaderInternal* _shaderInternal)
{
	...
	std::unordered_map<uint32_t, std::vector<vk::DescriptorSetLayoutBinding>> DescriptorSetLayoutBindings;
	RelflectDescriptorLayout(_device, _shaderInternal, &DescriptorSetLayoutBindings);

	_shaderInternal->descriptorSetLayouts.resize(DescriptorSetLayoutBindings.size());
	size_t i = 0;
	for (const auto& [setIndex, bindings] : DescriptorSetLayoutBindings)
	{
		vk::DescriptorSetLayoutCreateInfo layoutInfo = {};
		layoutInfo.sType = vk::StructureType::eDescriptorSetLayoutCreateInfo;
		layoutInfo.bindingCount = static_cast<uint32_t>(DescriptorSetLayoutBindings.at(setIndex).size());
		layoutInfo.pBindings = DescriptorSetLayoutBindings.at(setIndex).data();
...
void VulkanShaderManager::AllocDescriptorSet(const std::string& _shaderName,uint32_t _descriptorSetLayout , vk::DescriptorSet* _descriptorSet, uint32_t _descriptorSetCount, vk::Device _device)
{
	const ShaderInternal* shaderInternal = GetShader(_shaderName);

	std::vector<vk::DescriptorSetLayout> layouts(_descriptorSetCount, shaderInternal->descriptorSetLayouts[_descriptorSetLayout]);
	...
	uint32_t _descriptorSetCount, vk::Device _device)
{
	...
	if (shaderInternal == nullptr || shaderInternal->descriptorPool == VK_NULL_HANDLE)
		return;
	...
	_DescriptorSetLayoutBindings)
{
	...
	for (size_t i = 0; i < shaderStageInfos.size(); i++)
	{
		...
		for (size_t j = 0; j < descriptorBindingCount; j++)
		{
			...
			auto it = _DescriptorSetLayoutBindings->find(descriptorBindingReflected->set);
			if (it == _DescriptorSetLayoutBindings->end())
			{
				_DescriptorSetLayoutBindings->insert({ descriptorBindingReflected->set , {} });
				it = _DescriptorSetLayoutBindings->find(descriptorBindingReflected->set);
			}

			vk::DescriptorSetLayoutBinding DescriptorSetLayoutBinding = {};

			DescriptorSetLayoutBinding.binding = descriptorBindingReflected->binding;
			DescriptorSetLayoutBinding.descriptorType = static_cast<vk::DescriptorType>(descriptorBindingReflected->descriptor_type);
			DescriptorSetLayoutBinding.descriptorCount = descriptorBindingReflected->count;
			DescriptorSetLayoutBinding.stageFlags = static_cast<vk::ShaderStageFlags>(spvReflectBlockVariable->shader_stage);
