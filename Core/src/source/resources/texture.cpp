...
void Texture::CreateTextureFromFile(const fs::path& _path)
{
    ...
    const uint32_t widht = static_cast<uint32_t>(textureSize.x);
    const uint32_t height = static_cast<uint32_t>(textureSize.y);
    ...
    RHI::GetInstance().GenerateMimpMap(m_ImageHandle, RHIFormat::R8G8B8A8_SRGB, textureSize.x, textureSize.y, m_MipLevel);
